...
def planet_migration (gas,planetLoc,planetMass,time,rhopl):
    ...
    #Type I migration
    mgas = disk.mu *cgs.mp
    qr=-0.14493*disk.dot_Mg(disk.time)*cgs.gC*disk.mcp*(-0.206349*planetLoc**(5.5)+planetLoc**4.5*disk.rout)/disk.alpha/planetLoc**(8.5)/disk.rout/np.sqrt(cgs.kB*(disk.dot_Mg(time)*cgs.gC*disk.mcp/planetLoc**3/cgs.sigmaSB)**(1/4)/mgas) #pressure gradient
...
def init_planets ():
    ...
    [23.12.05]:it's probably better to format this as dictionaries
    """
